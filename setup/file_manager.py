...
class FileManager:
    ...
    def check_yaml_specification(self) -> typing.Any:
        required_spec: typing.Set[str] = {"config", "data", "pipeline"}
        required_config: typing.Set[str] = {"error"}
        required_data: typing.Set[str] = {"body", "band"}
        required_band: typing.Set[str] = {"input", "name"}
        required_pipeline: typing.Set[str] = {"step"}

        # Check for top level keys
        self.check_yaml_block("YAML file", self.spec, required_spec)
        ...
        if "niter" in self.spec["config"]:
            niter: int = self.spec["config"]["niter"]
            if not isinstance(niter, int):
                print("[ERROR] Number of iterations 'niter' should be an integer.")
                exit()
            if niter < 1:
                print("[ERROR] Number of iterations should be non-zero positive.")
                exit()
        else:
            niter: int = int(1)
            self.spec["config"]["niter"]: int = niter
